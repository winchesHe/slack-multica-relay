...
模型快照查询和旧兼容代码已删除。消费端只读当前 task 的完整消息数组，校验连续 seq 和 issue/task 范围；缺口、空日志、协议异常、超过 4 MiB 或 10000 条时隐藏日志统计。Skills 只支持已验证的简单 cat 读取与唯一相邻结果，无法配对或无法解析的读取隐藏 Skills。最终 footer 保存 90 天供更新重试复用，原始日志不落盘。迟到 usage/日志最多补查三次；登记索引、15 分钟延迟检查和每日 Cron 接续 QStash 重试耗尽后的恢复。错误累计 12 次或运行持续未结束超过七天后停止并保留原因；正文、作者或范围不满足要求时直接停止。长回复拆分与跨系统 exactly-once 不在保证范围。详见 Footer 运维文档。
