...
2. 执行 `rtk proxy python3 <本 Skill 路径>/scripts/run_context.py --issue <当前 Issue UUID> --issue-identifier <当前任务编号> --workspace-slug <当前工作区 slug> --app-url <Multica 网页根地址> --output <任务私有目录/context.json>`。编号复用本轮已有 `multica issue get` 结果中的 `identifier`，并核对该结果的 `id`、`workspace_id` 与当前任务一致；不从标题、历史回复或 run ID 推断编号。工作区 slug 和网页地址使用本轮已确认的配置；本项目 GRM 的已知值为 `grm` 和 `https://multica.devops.moego.dev`，仅在当前工作区确认为 GRM 时使用。缺少其中任一项时省略相应可选参数，不为 Footer 额外查询。脚本使用真实 `MULTICA_TASK_ID`，校验当前 run 的任务归属，查询所属 Agent 配置和 run-messages，输出 `statistics`、带日志序号的 `code_evidence`，以及信息完整时的 `issue_identifier`、`issue_url`。链接使用已校验的 issue UUID；脚本不读取对话上下文，不用 run ID 拼链接，不选择最近一次 run，不轮询等日志补齐。
