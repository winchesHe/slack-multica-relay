...
4. 读取 [Footer 展示](references/footer-display.md)，按其规则与完整 JSON 示例分别组装最终正文、Footer context blocks 和包含同样信息的 fallback。Footer 的 `context.elements` 文本对象必须使用 `mrkdwn`；Multica 编号与 PR 编号必须写成 `<URL|标题>`，分支使用行内代码。不能使用 `plain_text` 承载 Footer，也不能把 fallback 中的“编号 + 裸 URL”直接复用到 Footer。发送前检查实际 blocks 文件及 dry-run 请求中的这些字段，确认链接目标与采集证据一致；预检成功本身不代表格式符合要求。
