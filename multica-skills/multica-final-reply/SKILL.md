...
Agent 配置 `MULTICA_APP_URL` 为网页根地址、`MULTICA_WORKSPACE_SLUG` 为工作区 slug，供任务隔离环境使用。资料脚本依赖已认证的 Multica CLI，以及 `MULTICA_SERVER_URL`、`MULTICA_WORKSPACE_ID`、`MULTICA_TASK_ID`。发送凭据、身份预检、消息格式和错误处理全部遵循现有 slack Skill；本 Skill 不提供发送脚本、回执存储或消息更新服务。
