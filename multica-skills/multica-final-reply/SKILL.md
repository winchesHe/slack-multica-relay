...
4. 读取 [Footer 展示](references/footer-display.md)，分别组装正文、Footer 与同信息的 fallback。Footer 的 `context.elements` 文本对象必须使用 `mrkdwn`，链接使用 `<URL|标题>`，分支使用行内代码；不要直接复用 fallback 的“编号 + 裸 URL”。发送前核对实际 blocks 文件：文本类型、链接包装及目标均正确，编号旁不重复裸 URL；dry-run 成功不能代替此检查。
