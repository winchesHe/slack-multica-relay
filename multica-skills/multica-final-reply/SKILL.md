...
5. 使用 `scripts/final_reply.py send --bundle <输出目录/bundle.json> --channel <原 channelId> --thread-ts <根 threadTs>` 一次发送。`--dry-run` 只预检。Skill 自带的 `slack_sender.py` 调用 Runtime 的 `RELAY_SLACK_CLI`，固定 `SLACK_REPLY_ACTOR`，持久化发送回执。

所有命令通过 `rtk proxy python3` 执行。发送后结束回复流程，不再追加或更新 Footer。重试必须使用同一 bundle；结果不明时核对回执与原线程，不换入口、删回执或重新准备后重发。

## 展示与数据口径

- 正文后使用 context 展示耗时、可确认的模型、非零 tools 和 skills；再按 PR 展示仓库名（仅 repo，不展示 owner）、分支和 PR 链接。查询、去重和链接仍使用完整 owner/repo。多 PR 去重，保留多仓库；没有 PR 时不显示空行。
...
Runtime 依赖：已认证的 `multica`、`gh` CLI，显式 `MULTICA_SERVER_URL`、`MULTICA_WORKSPACE_ID`、`MULTICA_TASK_ID`，以及 `RELAY_SLACK_CLI`、`SLACK_TEAM_ID`、`SLACK_REPLY_ACTOR`。`RELAY_RECEIPT_DIR` 指向跨运行保留的私有目录，默认 `~/.local/state/slack-multica-relay`。缺失配置应报告，不能绕过脚本直接发送。
