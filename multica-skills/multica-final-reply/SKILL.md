...
2. 执行 `rtk proxy python3 <本 Skill 路径>/scripts/run_context.py --issue <当前 Issue UUID> --output <任务私有目录/context.json>`。脚本按真实 `MULTICA_TASK_ID` 采集当前 run、Agent 配置和日志，输出统计、成果证据与任务链接。任务编号和工作区 slug 缺失时自动查询，网页根地址读取当前 CLI 配置的 `app_url`；已有值可通过 `--issue-identifier`、`--workspace-slug`、`--app-url` 传入，优先使用。信息仍不完整时省略链接，继续返回统计。脚本不选择其他 run，不轮询等日志补齐。
